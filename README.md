...
Относительно свежая версия Ирины доступна в виде Docker-образа (на данный момент только для Linux+amd64) и может быть
запущена следующей командой:

```shell
docker run -it --publish 8086:8086 -v "$HOME/irene:/irene" alexeybond/irene:latest
```
...
изменив файл `$IRENE_HOME/config/face_web_server.yaml`.*

Приложение будет хранить свои настройки в папке, смонтированной к пути `/irene` (в примере выше - `$HOME/irene`).
